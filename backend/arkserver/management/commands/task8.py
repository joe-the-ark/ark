...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        for power in sorted(result.keys(), key=lambda x:result[x]['count'], reverse=True):
            ...
            print(f"[{get_names(ubung_3_term_list_i18n, power)}]: {result[power]['count']} ({','.join(result[power]['players'])}) <{round(result[power]['u5_sum']/result[power]['u5_cnt'], 1)}>")
            for drainer in top_5:
                print(f"\t[{get_names(ubung_1_term_list_i18n, drainer)}]: {drainers[drainer]['count']} ({','.join(drainers[drainer]['players'])}) <{round(drainers[drainer]['u5_sum']/drainers[drainer]['u5_cnt'], 1)}>")
            print()
    """
